...
class BrandAdmin extends Simpla
{
  ...
  function fetch()
  {
		...
		if($this->request->method('post'))
		{
			...
			$brand->article_title = $this->request->post('article_title');
			...
			$brand->is_popular       = $this->request->post('is_popular');
			...
			else
			{
                ...
                if (!empty($edit)) {
                    ...
                    $this->log->add_log($brand, 's_brands', $brand->id, $manager->login);
                    ...
                    if (!empty($image['name']) && in_array(strtolower(pathinfo($image['name'], PATHINFO_EXTENSION)), $this->allowed_image_extentions)) {
                        ...
                        $this->brands->update_brand($brand->id, ['image' => $image['name']]);
                    }
                }
