...
class UserView extends View
{
    function fetch()
    {
        ...
        if ($this->request->method('post') && $this->request->post('name')) {
            ...
            if ($images = $this->request->files('avatar')) {

                if ($image_name = $this->image->upload_avatar($images['tmp_name'], $images['name'])) {

                    $this->users->update_user($this->user->id, array(
                        'avatar' => $image_name
                    ));

                    $this->user = $this->users->get_user(intval($this->user->id));
                    $this->design->assign('avatar', $image_name);
            ...
            $this->design->assign('name', $this->user->name);
            $this->design->assign('email', $this->user->email);
            $this->design->assign('avatar', $this->user->avatar);
        }
